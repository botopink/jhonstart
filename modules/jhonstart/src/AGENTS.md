...
| `server.bp` | **compiled** | the **request** (front 28, promoted from `server.d.bp`): `type RequestData(method, path, params, query, headers, cookies)` with `param`/`queryParam`/`header`/`cookie` — each a plain `string`, `""` when absent, all four through front 26's `pairValue`; `request() -> @Component<ElementBase, RequestData>` (a hook: the return is the effect) over six dual-target cells, the one writer `fillRequest(method, path, params, query, headers, cookies)`, the two shortcuts `cookies()`/`headers()`, and `renderServerComponent(component: fn() -> @Task<Element>) -> @Task<string>` (and `renderComponent` for a `fn() -> @Component<ElementBase, Element>` thunk) — one `await`, then a synchronous render, over an **unstarted thunk**. The pair-shaped values travel querystring-encoded and are decoded with `router.decodePairs`. The `Http` phantom base and the `Request` behavior did NOT come along (every hook anchors at `ElementBase`, botopink decisions 96/128). Ships no escaping (front 01's) and no parallel awaiting (front 02's) |
