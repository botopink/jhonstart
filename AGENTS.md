...
`examples/jhonstart-counter`, `jhonstart-html` and `jhonstart-todo` are listed as known broken (`expects 2 argument(s), got 1`); `examples/jhonstart-app` has no `botopink.json` and is not built.
